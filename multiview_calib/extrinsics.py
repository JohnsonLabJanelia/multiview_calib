...
def _print_relative_pose_info(
    F, Rd, td, pts1_undist, pts2_undist, verbose=2, print_prefix=""
):
    if verbose > 1:
        logging.info(
            "{}\tFundamental matrix:\n{}\t\t{}\n{}\t\t{}\n{}\t\t{}".format(
                print_prefix, print_prefix, F[0], print_prefix, F[1], print_prefix, F[2]
            )
        )
        logging.info(
            "{}\tRight camera position:\n{}\t\t{}".format(
                print_prefix, print_prefix, utils.invert_Rt(Rd, td)[1].ravel()
            )
        )
    if verbose > 0:
        logging.info(
            "{}\tResidual error: {}".format(
                print_prefix, residual_error(pts1_undist, pts2_undist, F)[0]
            )
        )
        logging.info(
            "{}\tSampson distance: {}".format(
                print_prefix, sampson_distance(pts1_undist, pts2_undist, F)[0]
            )
        )


def compute_relative_poses(
    view_tree, intrinsics, landmarks, method="8point", th=20, verbose=2, print_prefix=""
):
    relative_poses = {}
    for view1, view2 in view_tree:
        pts1, pts2, ids_common = _common_landmarks(
            landmarks[view1]["landmarks"],
            landmarks[view2]["landmarks"],
            landmarks[view1]["ids"],
            landmarks[view2]["ids"],
        ...
        Rd, td, F, pts1_undist, pts2_undist, tri, mask = compute_relative_pose(
            pts1, pts2, K1=K1, dist1=dist1, K2=K2, dist2=dist2, method=method, th=th
        )

        if verbose > 0:
            logging.info(
                "{}Computing relative pose of pair {}:".format(
                    print_prefix, [view1, view2]
                )
            )
            logging.info(
                "{}\t{} out of {} points considered outliers.".format(
                    print_prefix, sum(~mask), len(pts1_undist)
                )
            )
        _print_relative_pose_info(
            F, Rd, td, pts1_undist[mask], pts2_undist[mask], verbose, print_prefix
        )

        relative_poses[(view1, view2)] = {
            "F": F.tolist(),
            "Rd": Rd.tolist(),
            "td": td.tolist(),
...
def visualise_cameras_and_triangulated_points(
    views, minimal_tree, poses, triang_points, max_points=100, path=None
):
    ...
    for i, (view1, view2) in enumerate(reversed(minimal_tree)):
        ...
        points_3d = np.asarray(
            triang_points[(view1, view2)]["triang_points"], np.float64
        ).T
...
def concatenate_relative_poses(
    minimal_tree, relative_poses, method="cross-ratios", verbose=2, print_prefix=""
):
    ...
    poses = {
        pair0[0]: {"R": np.eye(3).tolist(), "t": np.zeros((3, 1)).tolist()},
        pair0[1]: {"R": relative_poses[pair0]["Rd"], "t": relative_poses[pair0]["td"]},
    }
    triang_points = {
        pair0: {
            "triang_points": relative_poses[pair0]["triang_points"],
    ...
    while len(pairs) > 0:
        ...
        for curr_pair in pairs:
            ...
            if method == "cross-ratios":
                relative_scale, relative_scale_std = estimate_scale_point_sets(
                    p3d_com, p3d_adj_com
                )
            ...
            if verbose > 0:
                logging.info(
                    "{}Concatenating relative poses for pair: {}".format(
                        print_prefix, curr_pair
                    )
                )
                logging.info(
                    "{}\t Relative scale to {} n_points={}: {:0.3f}+-{:0.3f}".format(
                        print_prefix,
                        adj_pair,
                        len(p3d_com),
                        relative_scale,
                ...
                logging.info(
                    "{}\t {} new position: {}".format(
                        print_prefix, second_view, utils.invert_Rt(R2, t2)[1].ravel()
                    )
                )

            if relative_scale < 0.1:
                logging.info(
                    "!!!! The relative scale for this pair is quite low. Something may have gone wrong !!!!"
                )
            ...
            p3d = np.dot(R_inv, np.float64(p3d).T * relative_scale) + np.reshape(
                t_inv, (3, 1)
            )

            poses[second_view] = {"R": R2.tolist(), "t": t2.tolist()}
            triang_points[curr_pair] = {"triang_points": p3d.T.tolist(), "ids": ids}

        if len(pairs) == len(unmatched_pairs):
            raise RuntimeError(
                "The following pairs are not connected to the rest of the network: {}".format(
                    unmatched_pairs
                )
            )
            break
...
def compute_relative_poses_robust(
    ...
    for view1, view2 in view_tree:
        if verbose > 0:
            logging.info("-------------------------------------------------")
            logging.info(
                "Computing robust relative pose for pair {}->{}".format(view1, view2)
            )
            logging.info("Initial relative pose:")

        view_tree = [[view1, view2]]
        relative_pose = compute_relative_poses(
            view_tree, intrinsics, landmarks, method, th, verbose, print_prefix=""
        )

        triangles = [
            x for x in list(nx.all_simple_paths(G, view1, view2, 2)) if len(x) == 3
        ][:max_paths]
        if len(triangles) == 0:
            logging.info(
                "There are no other way to reach view {} from {}.".format(view2, view1)
            )

        if verbose > 0:
            logging.info("Number of additional paths found: {}".format(len(triangles)))

        relative_pose_robust = {
            "Rd": [relative_pose[(view1, view2)]["Rd"]],
            "td": [relative_pose[(view1, view2)]["td"]],
        }
        for nodes in triangles:
            view_tree = [nodes[:2], nodes[1:]]
            _relative_pose = compute_relative_poses(
                view_tree, intrinsics, landmarks, method, th, verbose, print_prefix="\t"
            )
            pose, _ = concatenate_relative_poses(
                view_tree, _relative_pose, method_scale, verbose, print_prefix="\t"
            )
        ...
        F = fundamental_from_relative_pose(Rd, np.squeeze(td), K1, K2)
...
def global_registration(ba_poses, ba_points, landmarks_global):
    ...
    src, dst, ids_common = _common_landmarks(
        ba_points["points_3d"],
...
def visualise_global_registration(
    global_poses,
    landmarks_global,
    ba_poses,
    ba_points,
